...
export function renderQrPay(container) {
  ...
  function render() {
    container.innerHTML = `
      <style>
        ...
        .ripple {
            position: absolute;
            border: 2px solid rgba(var(--accent-rgb, 13, 185, 242), 0.3);
            border-radius: 50%;
            transform: scale(0);
      ...
      <div class="relative flex h-[100dvh] w-full flex-col bg-background-light dark:bg-background-dark max-w-md mx-auto overflow-hidden font-display shadow-2xl safe-area-bottom">
        ...
        <header class="pt-6 pb-4 px-6 flex items-center justify-between bg-transparent z-20 notch-safe-top">
          ...
          <button id="btn-flash" class="flex size-10 items-center justify-center rounded-full backdrop-blur-md transition-colors ${flashEnabled ? 'bg-primary text-background-dark shadow-[0_0_15px_var(--primary-glow)]' : 'bg-slate-900/40 dark:bg-slate-100/10 text-white hover:bg-slate-900/60'}" style="${mode === 'scan' ? '' : 'display:none'}">
            <span class="material-symbols-outlined font-fill">${flashEnabled ? 'flashlight_off' : 'flashlight_on'}</span>
          </button>
        </header>
  ...
  function renderMyQR() {
    ...
    return `
      ...
      <div class="relative z-10 flex flex-col items-center justify-center h-full w-full px-6 pt-6 pb-32 overflow-y-auto no-scrollbar">
  ...
  function attachEvents() {
    ...
    const btnFlash = container.querySelector('#btn-flash');
    if (btnFlash) {
  ...
  async function toggleFlash() {
    ...
    if (!capabilities.torch) {
      showToast('Thiết bị không hỗ trợ đèn Flash', 'error');
      return;
    }
    ...
    try {
      ...
      render(); // Refresh UI to show active state
    } catch (err) {
      console.error("Flash error:", err);
      showToast('Không thể bật đèn Flash', 'error');
      flashEnabled = false;
    }
  }
  ...
  function showQrPayModal(toUserId, externalData = null) {
    ...
    modalOverlay.querySelector('#btn-qr-confirm').addEventListener('click', async () => {
      ...
      if (res.ok) {
        ...
        sOverlay.innerHTML = `
          <div class="size-24 rounded-full bg-primary flex items-center justify-center shadow-[0_0_60px_var(--primary-glow)] mb-8 animate-[pop_0.5s_cubic-bezier(0.175,0.885,0.32,1.275)_forwards] success-aura">
          ...
          <p class="text-primary font-mono text-2xl font-bold mb-6 animate-[slide-up_0.5s_ease-out_0.2s_both]">- ${formatVND(Number(raw))} VND</p>
