            ...
            <div class="space-y-3">
              ${txs.map(tx => {
                ...
                <div class="tx-item flex items-center justify-between p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/50 cursor-pointer active:scale-95 transition-transform" data-tx-id="${tx.id}">
                  ...
                  <p class="font-bold ${isIncoming ? 'text-primary' : 'text-slate-900 dark:text-slate-100'} text-right">
...
<div class="relative">
<div class="flex size-20 items-center justify-center rounded-full bg-primary/20 text-primary success-aura">
<span class="material-symbols-outlined text-4xl font-bold">check_circle</span>
</div>
<div class="absolute -bottom-1 -right-1 flex size-8 items-center justify-center rounded-full ${isIncoming ? 'bg-primary' : 'bg-red-500'} text-white border-4 border-background-dark">
...
<h2 class="text-3xl font-bold tracking-tight pt-2 ${isIncoming ? 'text-primary' : ''}">${amountPrefix} ${formatVND(tx.amount).replace('₫', '')} VND</h2>
...
function getTxIcon(type) {
  return { transfer: 'swap_horiz', topup: 'add_circle', bill: 'receipt_long', qr_pay: 'qr_code_scanner' }[type] || 'payments';
}
