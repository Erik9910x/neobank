...
function handleUpdatePreferences({ theme, accentColor, accentGradient, fontSize }) {
  requireAuth();
  const user = getUserById(currentUserId);
  if (theme !== undefined) user.theme = theme;
  if (accentColor !== undefined) user.accentColor = accentColor;
  if (accentGradient !== undefined) user.accentGradient = accentGradient;
  if (fontSize !== undefined) user.fontSize = fontSize;
  saveDB();
  return { ok: true, preferences: { theme: user.theme, accentColor: user.accentColor, accentGradient: user.accentGradient, fontSize: user.fontSize } };
}
