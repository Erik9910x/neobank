...
export function renderBottomNav() {
  ...
  nav.innerHTML = `
    <div class="grid grid-cols-5 items-center pt-2 pb-1 px-2">
      ...
      <a class="nav-link flex flex-col items-center justify-center gap-1" href="#transfer" data-route="transfer">
      ...
      <div class="relative -top-8 flex justify-center items-center w-full">
        <button id="nav-qr-btn" class="w-16 h-16 rounded-full bg-primary shadow-[0_8px_24px_rgba(var(--accent-rgb, 13, 185, 242), 0.4)] flex items-center justify-center border-4 border-background-dark active:scale-90 transition-transform">
          <span class="material-symbols-outlined text-background-dark text-4xl font-bold">qr_code_scanner</span>
        </button>
        <div class="absolute w-16 h-16 rounded-full bg-primary/20 blur-xl -z-10"></div>
      </div>
      <a class="nav-link flex flex-col items-center justify-center gap-1" href="#notifications" data-route="notifications">
      ...
      <a class="nav-link flex flex-col items-center justify-center gap-1" href="#profile" data-route="profile">
